...
import se.vidstige.jadb.managers.*;
...
/**
 * Updated version of jadb.PackageManager that now uses /data/local/tmp as the
 * temp install directory due to new security restrictions on Android 9+
 */
public class UpdatedPackageManager {
	private final JadbDevice device;

	public UpdatedPackageManager(JadbDevice device) {
		this.device = device;
	}
	...
	public List<Package> getPackages() throws IOException, JadbException {
		...
		try {
			input = new BufferedReader(
					new InputStreamReader(device.executeShell("pm", "list", "packages"), Charset.forName("UTF-8")));
			String line;
	...
	public void installWithOptions(File apkFile, List<? extends InstallOption> options)
	...
	public void launch(Package name) throws IOException, JadbException {
		InputStream s = device.executeShell("monkey", "-p", name.toString(), "-c", "android.intent.category.LAUNCHER",
				"1");
		s.close();
	}
